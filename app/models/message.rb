class Message < ActiveRecord::Base
  ...
  def self.getConnections(user,search)
	...
	if search
		...
		user_connections = Connection.joins("INNER JOIN users ON users.id = connections.con_id").select("connections.*, users.* , connections.id as cid").where(:user_id=>user.id).where("connections.status" => 2).where("users.username LIKE ? " , "%#{search}%" ).order('connections.id DESC')
  ...
  def self.getUserMessages(user,selected_user_id)
  	user_messages = Message.where("(messages.sender_id = ? AND messages.receiver_id = ?) OR ( messages.receiver_id = ? AND messages.sender_id = ?)" , user.id,selected_user_id,user.id,selected_user_id).order('messages.created_at ASC')
