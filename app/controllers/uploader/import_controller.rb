class Uploader::ImportController < ApplicationController
  ...
  def white_glove_service_request
    ...
    if params[:fileToUpload]
      ...
      DxMailer.white_glove_service_email('muhammad.jahanzaib@discretelogix.com',"http://"+request.host_with_port+"/system/marketrex_uploads/"+@updated_file_name,"http://"+request.host_with_port+"/uploader/import/new/"+encrypted_data).deliver_now
      redirect_to uploader_import_index_path
    else
