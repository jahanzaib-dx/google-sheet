  ...
  def index
    ...
    @activity_logs = activity_logs.each do |activity|
      ...
      if activity.comptype == 'lease'
        ...
        activity_comp.net_effective_per_sf = number_with_precision(activity_comp.net_effective_per_sf,:precision => 2)
      else
...
#       
  ...
  def lockData activity, unlockFields
    ...
    if activity.comptype == 'lease'
      ...
      activity_comp.net_effective_per_sf = if unlockFields.include?"net_effective_per_sf" then activity_comp.net_effective_per_sf else "-0.0" end
